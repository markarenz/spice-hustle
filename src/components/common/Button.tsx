...
const Button: React.FC<Props> = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`
      relative py-2 px-4 rounded-lg border-2 group block w-full lg:w-auto lg:inline-block lg:inline
      ${
        variant === 'primary' &&
        !disabled &&
        'border-gray-800 hover:scale-100 lg:hover:scale-110 transition-all text-gray-100 bg-gradient-to-b from-yellow-500 to-red-900 ring-0 ring-gray-100 hover:ring-2'
      }
      ${
        variant === 'secondary' &&
        `${
